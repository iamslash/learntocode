...
class Solution {
    public int minDifficulty(int[] jobDifficulty, int days) {
        int n = jobDifficulty.length, MAX_SUM = 300_000;
        if (n < days) {
            return -1;
        }
        // C[i]: min sum of job difficulty starts with i-th job
        int[] C = new int[n+1];
        Arrays.fill(C, MAX_SUM);
        C[n] = 0;
        ...
        for (int d = 0; d < days; ++d) {
            for (int i = 0; i < n-d; ++i) {
                ...
                C[i] = MAX_SUM;
                for (int j = i; j < n-d; ++j) {
                    fronMaxJob = Math.max(fronMaxJob, jobDifficulty[j]);
                    C[i] = Math.min(C[i], fronMaxJob + C[j+1]);
                }
            }
        }        
        return C[0];
